...
func (m *MailNotification) SendWelcomeEmail(to string, username string, verifyURL string) error {
	...
	tmpl, err := template.New("welcome").Parse(hibiscusIM.WelcomeHTML)
	if err != nil {
...
func (m *MailNotification) SendVerificationCode(to, code string) error {
	tmpl, err := template.New("verification").Parse(hibiscusIM.VerificationHTML)
	if err != nil {
	...
	msg += fmt.Sprintf("From: %s\r\n", m.Config.From)
	msg += fmt.Sprintf("To: %s\r\n", to)
	msg += fmt.Sprintf("Subject: %s\r\n", "Your HibiscusIM Veritifaction Code")
	msg += "\r\n" + body.String()
