...
type AdminObject struct {
	...
	GetDB            hibiscusIM.GetDB            `json:"-"`
	BeforeCreate     hibiscusIM.BeforeCreateFunc `json:"-"`
	BeforeRender     hibiscusIM.BeforeRenderFunc `json:"-"`
	BeforeUpdate     hibiscusIM.BeforeUpdateFunc `json:"-"`
	BeforeDelete     hibiscusIM.BeforeDeleteFunc `json:"-"`
...
func GetHibiscusAdminObjects() []AdminObject {
	...
	iconUser, _ := hibiscusIM.EmbedStaticAssets.ReadFile("static/img/icon_user.svg")
	...
	return []AdminObject{
		{
			...
			Orders:      []hibiscusIM.Order{{"UpdatedAt", hibiscusIM.OrderOpDesc}},
...
func WithAdminAuth() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if user == nil {
			...
			if signUrl == "" {
				hibiscusIM.AbortWithJSONError(ctx, http.StatusUnauthorized, errors.New("login required"))
			} else {
		...
		if !user.IsStaff && !user.IsSuperUser {
			hibiscusIM.AbortWithJSONError(ctx, http.StatusForbidden, errors.New("forbidden"))
			return
		}
		ctx.Next()
	}
...
func HandleAdminJson(c *gin.Context, objects []*AdminObject, buildContext AdminBuildContext) {
	...
	siteCtx := hibiscusIM.GetRenderPageContext(c)
	if buildContext != nil {
...
func (obj *AdminObject) RegisterAdmin(r gin.IRoutes) {
	r = r.Use(func(ctx *gin.Context) {
		if obj.AccessCheck != nil {
			...
			if err != nil {
				hibiscusIM.AbortWithJSONError(ctx, http.StatusForbidden, err)
				return
			}
		}
		ctx.Next()
...
func (obj *AdminObject) MarshalOne(c *gin.Context, val interface{}) (map[string]any, error) {
	...
	if obj.ViewOnSite != nil {
		result["_adminExtra"] = map[string]any{
			"viewOnSite": obj.ViewOnSite(hibiscusIM.GetDbConnection(c, obj.GetDB, false), c, val),
		}
	}
...
func (obj *AdminObject) QueryObjects(session *gorm.DB, form *hibiscusIM.QueryForm, ctx *gin.Context) (r AdminQueryResult, err error) {
	for _, v := range form.Filters {
		if q := v.GetQuery(); q != "" {
			if v.Op == hibiscusIM.FilterOpLike {
			...
			} else if v.Op == hibiscusIM.FilterOpBetween {
	...
	for i := 0; i < vals.Elem().Len(); i++ {
		...
		if obj.BeforeRender != nil {
			db := hibiscusIM.GetDbConnection(ctx, obj.GetDB, false)
...
func (obj *AdminObject) handleQueryOrGetOne(c *gin.Context) {
	...
	db, form, err := hibiscusIM.DefaultPrepareQuery(hibiscusIM.GetDbConnection(c, obj.GetDB, false), c)
	if err != nil {
...
func (obj *AdminObject) handleUpdate(c *gin.Context) {
	...
	if err != nil {
		hibiscusIM.AbortWithJSONError(c, http.StatusNotFound, errors.New("not found"))
		return
	}
