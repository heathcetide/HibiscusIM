...
func (h *Handlers) handleUserSignupPage(c *gin.Context) {
	ctx := hibiscusIM.GetRenderPageContext(c)
...
func (h *Handlers) handleUserResetPasswordPage(c *gin.Context) {
	c.HTML(http.StatusOK, "reset_password.html", hibiscusIM.GetRenderPageContext(c))
}
...
func (h *Handlers) handleUserSigninByEmail(c *gin.Context) {
	...
	// 校验验证码
	if form.Code == "" {
		hibiscusIM.AbortWithJSONError(c, http.StatusBadRequest, errors.New("verification code is required"))
		return
	}
	...
	if err != nil {
		hibiscusIM.AbortWithJSONError(c, http.StatusForbidden, err)
		return
	}
...
func (h *Handlers) handleUserSignin(c *gin.Context) {
	...
	if form.Password == "" && form.AuthToken == "" {
		hibiscusIM.AbortWithJSONError(c, http.StatusBadRequest, errors.New("empty password"))
		return
	}
	...
	if form.Password != "" {
		user, err = models.GetUserByEmail(db, form.Email)
		if err != nil {
			hibiscusIM.AbortWithJSONError(c, http.StatusBadRequest, errors.New("user not exists"))
			return
		}
		if !models.CheckPassword(user, form.Password) {
			hibiscusIM.AbortWithJSONError(c, http.StatusUnauthorized, errors.New("unauthorized"))
			return
		}
	} else {
...
func (h *Handlers) handleUserSignupByEmail(c *gin.Context) {
	...
	go func() {
		err = db.Create(&notification.InternalNotification{
			UserID:    user.ID,
			Title:     "welcome to use the hibiscusIM",
			Content:   "welcome to join hibiscusIM，now, you can create your first assistant",
			Read:      false,
			CreatedAt: time.Now(),
		}).Error
		if err != nil {
